...
{
    public class BooksManagementModel : PageModel
    {
        private readonly IHttpClientFactory _httpClientFactory;

        public BooksManagementModel(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IActionResult> OnPostSearchBooksAsync()
        {
            ...
            if (response is not null)
            {
                ...
                {
                    var responseString = await response.Content.ReadAsStringAsync();
                    Books = [JsonConvert.DeserializeObject<Book>(responseString)];
                }
                else
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.Forbidden)
